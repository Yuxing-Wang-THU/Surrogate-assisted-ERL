...
class SSNE:
    def __init__(self, args):
        ...
        self.rl_policy = None
        self.best_real_policy = None
        self.selection_stats = {'elite': 0, 'selected': 0, 'discarded':0, 'total':0.0000001}
        self.surro_selection_stats = {'elite': 0, 'selected': 0, 'discarded':0, 'total':0.0000001}
    ...
    def epoch(self, pop, fitness_evals, button=None):
        ...
        if button == "real":
            if self.rl_policy != None: #RL Transfer happened
                ...
                self.rl_policy = None
                # info_dict = {'elite':self.selection_stats['elite']/self.selection_stats['total'],
                #                 'selected': self.selection_stats['selected']/self.selection_stats['total'],
                #                 'discarded':self.selection_stats['discarded']/self.selection_stats['total'],
                #                 'frames':frames,
                #                 'episodes':episodes}
        ...
        if button == "surro":
            if self.best_real_policy != None: #RL Transfer happened
                ...
                elif self.best_real_policy in offsprings: self.surro_selection_stats['selected'] += 1.0
                elif self.best_real_policy in unselects: self.surro_selection_stats['discarded'] += 1.0
                self.best_real_policy = None
                
                # info_dict = {'elite':self.surro_selection_stats['elite']/self.surro_selection_stats['total'],
                #              'selected': self.surro_selection_stats['selected']/self.surro_selection_stats['total'],
                #              'discarded':self.surro_selection_stats['discarded']/self.surro_selection_stats['total'],
                #              'frames':frames,
                #              'episodes':episodes}
                # best_logger.writerow(info_dict)
                # best_log.flush() 
        ...
        print("New elites index:", new_elitists)
        print("returned unselects:", unselects)
        return new_elitists[0], new_elitists, unselects
